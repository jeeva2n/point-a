...
const updateProduct = async (id, productData, files = [], deleteImages = []) => {
  ...
  try {
    ...
    const { mainImageId, mainImageIndex } = productData;
    if (mainImageId !== undefined && mainImageId !== null) {
      ...
      const [mainImageRow] = await connection.query(
      ...
      if (allImages.length > 0) {
        ...
        await connection.query('UPDATE product_images SET is_main = 0 WHERE product_id = ?', [id]);
        await connection.query('UPDATE product_images SET is_main = 1 WHERE id = ?', [allImages[mainIdx].id]);
        await connection.query('UPDATE products SET image_url = ? WHERE id = ?', [allImages[mainIdx].image_url, id]);
      }
    }

    // Parse JSON fields for update
    const { materials, specifications, features, ...otherData } = productData;
...
const deleteProduct = async (id) => {
  ...
  try {
    ...
    return images; // Return images for file cleanup
  } catch (error) {
    await connection.rollback();
    throw error;
  } finally {
